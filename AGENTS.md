...
|-- apps/game/         # RPG-JS map, village resource loop (CropPlot/Tree/Mine/OrderBoard)
...
| Quest logic             | `apps/game/src/modules/village/events/`              | CropPlot/Tree/Mine/OrderBoard village state machine. |
| RPG-JS map wiring       | `apps/game/src/modules/village/map-adapter.ts`       | Event placement for `village` map.                   |
...
| `CropPlotFactory`     | RPG-JS event factory | `apps/game/src/modules/village/events/` | map             | Plant/water/harvest Popberry cycle.    |
| `TreeFactory`         | RPG-JS event factory | `apps/game/src/modules/village/events/` | map             | Chop for Whittlewood Log.              |
| `MineFactory`         | RPG-JS event factory | `apps/game/src/modules/village/events/` | map             | Mine rocks for Ochrux Matrix.          |
| `OrderBoardFactory`   | RPG-JS event factory | `apps/game/src/modules/village/events/` | map             | Fulfill village orders, awards points. |
...
- Core loop is fixed: guest player -> farm village -> plant/water/harvest crops -> chop trees -> mine rocks -> fulfill orders -> earn off-chain points -> guest badge -> optional wallet proof -> leaderboard.
...
- For gameplay, map, sprites, RPG-JS config, village events, NPCs, resource nodes, input, or build-output changes, run `npm run build:vercel && npm run test:game:render && npm run test:game:ai`.
...
- For game QA, verify the actual RPG-JS scene: visible map art, player sprite, NPC sprite, resource node sprites/markers (crop plot, tree, mine, order board), keyboard movement, Space interaction, mobile GUI/A-button behavior if enabled, and village progress text/notifications.
