...
AI-native Web3 pixel quest RPG where players complete cozy resource quests, earn off-chain points, and optionally prove completion with a safe readable wallet signature — no token economy required.
...
ai, game, web3, rpg, pixel-art, supabase, rpgjs, no-token

## Safety line

Guest-first. No token. No RMT. No wallet required. Optional proof uses readable signature only.
